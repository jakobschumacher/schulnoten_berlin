...
# We'll use the CSV data to get the correct schul_nr for each school
schulen_with_ids <- schulen |>
  dplyr::filter(bezirk == "Pankow" | bezirk == "Lichtenberg") |>
  dplyr::select(name, schultyp, bezirk, website, lon, lat)

# Create a reference table for schul_nr from CSV data
schul_nr_reference <- data |>
  dplyr::select(name, bezirk, schul_nr) |>
  dplyr::distinct()

# Join map data with schul_nr reference
schulen_with_ids <- schulen_with_ids |>
